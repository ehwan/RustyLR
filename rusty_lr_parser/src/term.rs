...
impl TermType {
    pub fn enum_index(&self) -> usize {
        match self {
            ...
            TermType::ParenGroup(_) => 15,
            TermType::BraceGroup(_) => 16,
            TermType::BracketGroup(_) => 17,
            TermType::OtherGroup(_) => 18,
            TermType::Literal(_) => 19,
            TermType::Equal(_) => 20,
            TermType::Plus(_) => 21,
            TermType::Star(_) => 22,
            TermType::Question(_) => 23,
            TermType::OtherPunct(_) => 24,
            TermType::Eof => 25,
        }
    }
    pub fn stream(self) -> TokenStream {
        match self {
            ...
            TermType::ErrorType(punct_ident) => {
                let (punct, ident) = punct_ident.unwrap();
                quote! { #punct #ident }
            }
            ...
            TermType::ParenGroup(group) => group.unwrap().to_token_stream(),
            TermType::BraceGroup(group) => group.unwrap().to_token_stream(),
            TermType::BracketGroup(group) => group.unwrap().to_token_stream(),
            TermType::OtherGroup(group) => group.unwrap().to_token_stream(),
    ...
    pub fn span(&self) -> Option<Span> {
        match self {
            ...
            TermType::ParenGroup(group) => group.as_ref().map(|g| g.span()),
            TermType::BraceGroup(group) => group.as_ref().map(|g| g.span()),
            TermType::BracketGroup(group) => group.as_ref().map(|g| g.span()),
            TermType::OtherGroup(group) => group.as_ref().map(|g| g.span()),
...
impl std::fmt::Display for TermType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            TermType::ParenGroup(_) => write!(f, "<ParenGroup>"),
            TermType::BraceGroup(_) => write!(f, "<BraceGroup>"),
            TermType::BracketGroup(_) => write!(f, "<BracketGroup>"),
            TermType::OtherGroup(_) => write!(f, "<OtherGroup>"),
